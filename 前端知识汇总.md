...
1. 性能优化
   1. 减少http请求次数: 合并静态资源, 雪碧图, 强缓存, localStorage/sessionStorage, ajax缓存, combo服务, 小图片base64内嵌
   1. 减少http请求大小: 压缩静态资源, gzip, 协商缓存, 减小cookie及cookie隔离, 压缩比更高的图片(webp), 根据屏幕尺寸返回适合尺寸图片
   1. 优化网络加载: 分域CDN, 减少重定向, 预加载, dns预解析
   1. 根据渲染原理优化: css放头部, js放脚部, 避免css import, 阻塞渲染的css/js合理拆分延迟加载, 图片懒加载
   1. 提升代码运行性能: 避免重排(预设图片尺寸, 避免width,left等修改), 减少js动画, css动画, GPU加速, 避免使用table/iframe
   1. 使用新技术: http2多路复用, PWA, ServiceWorker
